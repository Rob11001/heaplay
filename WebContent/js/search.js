...
function createDiv(bean,container,typeOfSearch) {
	...
	const playlistDiv= typeOfSearch == "playlist" ? "<div class='playlist'><a href='/heaplay/user/"+bean.authorName+"/playlist/"+bean.name.replace(/\s/g,'')+"?id="+bean.id+"'><img width='100px' alt='Non trovata' src='/heaplay/getImage?id=" + bean.tracks[0].id + "'></a><div class='playlist-info'><span><a href='/heaplay/user/"+bean.authorName+"/playlist/"+bean.name.replace(/\s/g,'')+"?id="+bean.id+"'>"+bean.name+"</a></span><br><span><a href='/heaplay/user/"+bean.authorName+"'> di "+bean.authorName+"</a></span></div></div>" : "";
